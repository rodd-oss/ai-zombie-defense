...
// versions:
//   sqlc v1.30.0
// source: leaderboards_weekly.sql
...
func (q *Queries) GetWeeklyLeaderboard(ctx context.Context, db DBTX) ([]*GetWeeklyLeaderboardRow, error) {
	rows, err := db.QueryContext(ctx, getWeeklyLeaderboard)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
